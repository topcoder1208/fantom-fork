...
func New() *mugamboEpochsSealerModule {
	return &mugamboEpochsSealerModule{}
}

func (m *mugamboEpochsSealerModule) Start(block blockproc.BlockCtx, bs blockproc.BlockState, es blockproc.EpochState) blockproc.SealerProcessor {
	return &mugamboEpochsSealer{
		block: block,
		es:    es,
		bs:    bs,
	}
...
type mugamboEpochsSealer struct {
	block blockproc.BlockCtx
	es    blockproc.EpochState
	bs    blockproc.BlockState
}

func (s *mugamboEpochsSealer) EpochSealing() bool {
...
func (p *mugamboEpochsSealer) Update(bs blockproc.BlockState, es blockproc.EpochState) {
	p.bs, p.es = bs, es
}

// SealEpoch is called after pre-internal transactions are executed
func (s *mugamboEpochsSealer) SealEpoch() (blockproc.BlockState, blockproc.EpochState) {
	...
	s.bs.EpochGas = 0
	s.bs.EpochCheaters = mugambobft.Cheaters{}
