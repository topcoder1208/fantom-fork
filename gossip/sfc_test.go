...
//go:generate bash -c "cd ../../mugambo-sfc && git checkout c1d33c81f74abf82c0e22807f16e609578e10ad8"
//go:generate bash -c "docker run --name go-mugambo-sfc-compiler -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../mugambo-sfc:/src -w /src node:10.5.0 bash -c 'export NPM_CONFIG_PREFIX=~; npm install --no-save; npm install --no-save truffle@5.1.4' && docker commit go-mugambo-sfc-compiler go-mugambo-sfc-compiler-image && docker rm go-mugambo-sfc-compiler"
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../mugambo-sfc:/src -w /src go-mugambo-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; rm -f /src/build/contracts/*json; npm run build'"
...
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../mugambo-sfc:/src -w /src go-mugambo-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; sed -i s/runs:\\ 200,/runs:\\ 10000,/ /src/truffle-config.js; rm -f /src/build/contracts/*json; npm run build'"
//go:generate bash -c "cd ../../mugambo-sfc && git checkout -- truffle-config.js; docker rmi go-mugambo-sfc-compiler-image"
