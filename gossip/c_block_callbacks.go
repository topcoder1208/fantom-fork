...
func (s *Service) GetConsensusCallbacks() mugambobft.ConsensusCallbacks {
	return mugambobft.ConsensusCallbacks{
...
) mugambobft.BeginBlockFn {
	return func(cBlock *mugambobft.Block) mugambobft.BlockCallbacks {
		wg.Wait()
		start := time.Now()
...
func spillBlockEvents(store *Store, block *inter.Block, network mugambo.Rules) (*inter.Block, inter.EventPayloads) {
...
func mergeCheaters(a, b mugambobft.Cheaters) mugambobft.Cheaters {
	if len(b) == 0 {
		return a
	}
	if len(a) == 0 {
	...
	aSet := a.Set()
	merged := make(mugambobft.Cheaters, 0, len(b)+len(a))
	for _, v := range a {
