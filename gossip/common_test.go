...
func (env *testEnv) ApplyBlock(spent time.Duration, txs ...*types.Transaction) (receipts types.Receipts) {
	...
	process := beginBlock(&mugambobft.Block{
		Atropos: event.ID(),
	})
...
func (env *testEnv) callContract(
	...
	vmenv := vm.NewEVM(context, txContext, state, env.store.GetRules().EvmChainConfig(), mugambo.DefaultVMConfig)
...
// FilterLogs executes a log filter mugambotion, blocking during execution and
...
// SubscribeFilterLogs creates a background log filtering mugambotion, returning
