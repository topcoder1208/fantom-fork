...
func (s *Store) GetRules() mugambo.Rules {
	cfg := s.rlp.Get(s.table.Rules, []byte("c"), &mugambo.Rules{}).(*mugambo.Rules)
	return *cfg
}

func (s *Store) SetRules(cfg mugambo.Rules) {
	s.rlp.Set(s.table.Rules, []byte("c"), &cfg)
}

func (s *Store) GetGenesis() mugambo.Genesis {
	meatadata := s.GetMetadata()
	return mugambo.Genesis{
