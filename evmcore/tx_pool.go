...
const (
	...
	// that validating a new transaction remains a constant mugambotion (in reality
...
func (pool *TxPool) loop() {
	...
	for {
		// mugambo-specific gas price updates
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	// Ensure mugambo-specific hard bounds
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (replaced bool, err error) {
	...
	if uint64(pool.all.Count()+numSlots(tx)) > pool.config.GlobalSlots+pool.config.GlobalQueue {
		...
		// Otherwise if we can't make enough room for new one, abort the mugambotion.
...
func (pool *TxPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*txSortedMap) {
	...
	if dirtyAccounts != nil && reset == nil {
		...
		// the flatten mugambotion can be avoided.
...
func (pool *TxPool) reset(oldHead, newHead *EvmHeader) {
	// update chain config (mugambo-specific)
