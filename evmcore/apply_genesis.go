...
func ApplyGenesis(statedb *state.StateDB, g mugambo.Genesis, maxMemoryUsage int) (*EvmBlock, error) {
	mem := 0
...
func genesisBlock(g mugambo.Genesis, root common.Hash) *EvmBlock {
	block := &EvmBlock{
		EvmHeader: EvmHeader{
			Number:   big.NewInt(0),
			Time:     g.Time,
...
func MustApplyGenesis(g mugambo.Genesis, statedb *state.StateDB, maxMemoryUsage int) *EvmBlock {
